...
```{r}
hotsprings_canyon <- sb_protected_areas |> 
  filter(UNIT_NAME == "Hot Springs Canyon")

# Find distance between edges
hotsprings_dist <- st_distance(goleta, hotsprings_canyon)
```

```{r}
# Calculate the geometric center
hotsprings_centroid <- st_centroid(hotsprings_canyon)
goleta_centroid <- st_centroid(goleta)

hotsprings_dist_centroid <- st_distance(goleta_centroid, hotsprings_centroid)
```
